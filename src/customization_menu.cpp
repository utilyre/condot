...
static const Position POSITIONS[6] = {
    Position::TOP_LEFT, Position::TOP_RIGHT,   Position::LEFT,
    Position::RIGHT,    Position::BOTTOM_LEFT, Position::BOTTOM_RIGHT,
};

CustomizationMenu::CustomizationMenu(State* state, Event* initiateBattleEvent, Event* addPlayerEvent)
    : m_State(state), m_InitiateBattleEvent(initiateBattleEvent), m_AddPlayerEvent(addPlayerEvent),
      m_Dimensions(
          {(GetScreenWidth() - MENU_WIDTH) / 2.0f, (GetScreenHeight() - MENU_HEIGHT) / 2.0f, MENU_WIDTH, MENU_HEIGHT}),
      m_MenuButton("Back to Menu",
                   Rectangle{
                                   ...
                                   })
{
    for (size_t i = 0; i < 3; i++)
    {
        m_PlayerRows.emplace_back(
            Input("Name",
                  Rectangle{m_Dimensions.x + MENU_PADDING, m_Dimensions.y + MENU_PADDING + MENU_SPACING * (i + 1),
                            INPUT_NAME_WIDTH, INPUT_NAME_HEIGHT}),

            Input("Age", Rectangle{m_Dimensions.x + MENU_PADDING + INPUT_NAME_WIDTH + MENU_SPACING,
                                   m_Dimensions.y + MENU_PADDING + MENU_SPACING * (i + 1), INPUT_AGE_WIDTH,
                                   INPUT_AGE_HEIGHT}));
    }
}
...
void CustomizationMenu::Update()
{
    ...
    if (m_IncreasePlayersButton.Pressed() && m_PlayerRows.size() < MAX_PLAYERS)
    {
        ...
        m_PlayerRows.emplace_back(
            Input("Name",
                  Rectangle{m_Dimensions.x + MENU_PADDING, m_Dimensions.y + MENU_PADDING + MENU_SPACING * (len + 1),
                            INPUT_NAME_WIDTH, INPUT_NAME_HEIGHT}),

            Input("Age", Rectangle{m_Dimensions.x + MENU_PADDING + INPUT_NAME_WIDTH + MENU_SPACING,
                                   m_Dimensions.y + MENU_PADDING + MENU_SPACING * (len + 1), INPUT_AGE_WIDTH,
                                   INPUT_AGE_HEIGHT}));
    }
...
void CustomizationMenu::Render(const AssetManager& assets) const
{
    ...
    DrawTextEx(assets.PrimaryFont, TextFormat("%d players", m_PlayerRows.size()),
               Vector2{
               ...
               TEXT_FONT_SIZE, 1, BLACK);

    DrawTextEx(assets.PrimaryFont, m_ErrorMsg.c_str(),
               Vector2{
                   m_Dimensions.x + 0.5f * MENU_WIDTH - 0.15f * TEXT_FONT_SIZE * m_ErrorMsg.size(),
                   m_Dimensions.y + MENU_PADDING,
               },
               TEXT_FONT_SIZE, 1, RED);
...
void CustomizationMenu::Continue()
{
    ...
    for (const PlayerRow& row : m_PlayerRows)
    {
        ...
        try
        {
            ...
            players.emplace_back(name, age, COLORS[players.size()], POSITIONS[players.size()]);
        }
