...
Input::Input(const std::string& placeHolder, Rectangle dimensions)
    : m_PlaceHolder(placeHolder), m_Dimensions(dimensions), m_Hovered(false), m_Focused(false)
{
}
...
void Input::Update()
{
    ...
    if (m_Focused)
    {
        ...
        if (!m_Text.empty() && (IsKeyPressed(KEY_BACKSPACE) || IsKeyPressedRepeat(KEY_BACKSPACE)))
        {
            m_Text.pop_back();
        }
    }
...
{
    DrawRectangleRoundedLines(m_Dimensions, INPUT_ROUNDNESS, 0, INPUT_BORDER_THICKNESS, m_Focused ? RED : BLACK);

    DrawTextEx(assets.PrimaryFont, m_Text.empty() ? m_PlaceHolder.c_str() : m_Text.c_str(),
               Vector2{
                   m_Dimensions.x + INPUT_PADDING,
                   m_Dimensions.y + 0.5f * (m_Dimensions.height - INPUT_FONT_SIZE),
               },
               INPUT_FONT_SIZE, 1, m_Text.empty() ? GRAY : BLACK);
}
