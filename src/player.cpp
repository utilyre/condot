...
Player::Player(const std::string& name, int age, Color color, Position position)
    : m_Name(name), m_Color(color), m_Age(age), m_Position(position), m_IsPassed(false), m_Spy(0), m_Heroine(0),
      m_Drummer(false), m_Bishop(0), m_PassButton("SURRENDER", Rectangle(10, GetScreenHeight() - 100, 170, 90)),
      m_RevealCardsButton("Reveal", Rectangle{10, GetScreenHeight() - 200.0f, 170, 90})
{
}

void Player::Init(State* state, Season* season, bool* hideCards, Event* rotateTurnEvent, Event* restartBattleEvent,
...
void Player::Update()
{
    ...
    if (state != State::PLAYING_CARD && state != State::SCARECROW)
    {
        return;
    }
...
void Player::Render(const AssetManager& assets) const
{
    ...
    case Position::RIGHT:
        ...
        DrawTextPro(assets.PrimaryFont, m_Name.c_str(),
                    Vector2{Right.x + CardHeight * Scale, GetScreenHeight() / 2.0f + 20}, Vector2{0, 0}, 270, 40, 12,
                    BLACK);
        break;
    ...
    case Position::LEFT:
        ...
        DrawTextPro(assets.PrimaryFont, m_Name.c_str(),
                    Vector2{Left.x - CardHeight * Scale, GetScreenHeight() / 2.0f - 20}, Vector2{0, 0}, 90, 40, 12,
                    BLACK);
...
void Player::RenderCards(const AssetManager& assets, Vector2 cordinate, float rotation, float ratio) const
{
    ...
    if (m_Position == Position::BOTTOM_LEFT)
    {
        ...
        {
            DrawTextureEx(*m_HideCards ? card.GetAsset(assets) : c.GetAsset(assets), cordinate, rotation, ratio, WHITE);
            cordinate.x += CardWidth / 2.0 * Scale;
        }
    }
...
bool Player::PlayCard()
{
    ...
    if (!*m_HideCards && m_Position == Position::BOTTOM_LEFT)
    {
        ...
        {
            Rectangle LowerLayer = {BottomLeft.x + CardWidth / 2.0f * scale * index, BottomLeft.y,
                                    CardWidth / 2.0f * scale, CardHeight * scale};
            Rectangle UpperLayer = {BottomLeft.x + CardWidth / 2.0f * scale * index, BottomLeft.y, CardWidth * scale,
                                    CardHeight * scale};

            if ((CheckCollisionPointRec(GetMousePosition(), LowerLayer) ||
                 (CheckCollisionPointRec(GetMousePosition(), UpperLayer) && m_Cards.size() - 1 == index)) &&
...
bool Player::RetrieveCard()
{
    ...
    const float scale = MIN((Width / CardWidth), (Height / CardHeight)) * CardScale;
    const float Spacing = scale * (9 * CardWidth / 2.0f + CardWidth);
    const Vector2 BottomLeft{(Width / 2.0f) - (Spacing * 1.1f),
                             Height - (CardHeight * scale) * 1.3f - (scale * (CardHeight / 2.0f))};
    ...
    if (m_Position == Position::BOTTOM_LEFT)
    {
        ...
        {
            Rectangle LowerLayer = {BottomLeft.x + CardWidth / 2.0f * scale * index, BottomLeft.y,
                                    CardWidth / 2.0f * scale, CardHeight / 2.0f * scale};
            Rectangle UpperLayer = {BottomLeft.x + CardWidth / 2.0f * scale * index, BottomLeft.y, CardWidth * scale,
                                    CardHeight / 2.0f * scale};

            if ((CheckCollisionPointRec(GetMousePosition(), LowerLayer) ||
                 (CheckCollisionPointRec(GetMousePosition(), UpperLayer) && m_Row.size() - 1 == index)) &&
                IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
            {
                Add(index);
...
PlayerInfo::PlayerInfo(const std::string& name, int age, Color color, Position position)
    : name(name), age(age), color(color), position(position)
{
}

PlayerInfo::PlayerInfo(const Player& player)
    : name(player.GetName()), age(player.GetAge()), color(player.GetColor()), position(player.GetPosition())
{
}
