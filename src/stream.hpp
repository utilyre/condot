...
public:
    ...
    {
        WriteData((const char*)&v, sizeof(T));
    }

    template <typename T> void WriteObject(const T& v)
    {
        T::Serialize(*this, v);
    }

    template <typename T> void WriteOptional(const std::optional<T>& o)
    {
    ...
    template <typename T> void WriteVector(const std::vector<T>& v, bool writeSize = true)
    {
        if (writeSize)
        {
...
public:
    ...
    {
        ReadData((char*)&v, sizeof(T));
    }

    template <typename T> void ReadObject(T& v)
    {
        T::Deserialize(*this, v);
    }

    template <typename T> void ReadOptional(std::optional<T>& o)
    {
        bool hasValue;
        ReadRaw(hasValue);
    ...
    template <typename T> void ReadVectorInPlace(std::vector<T>& v)
    {
        if constexpr (std::is_trivial<T>())
        {
