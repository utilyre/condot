...
Game::Game()
    : m_Stopped(false), m_FavorMarkerChooserIndex(-1), m_HideCards(true), m_AutoSaveTimer(AUTOSAVE_INTERVAL, true),
      ...
      m_Map(&m_State, &m_Players, &m_Turn, &m_FavorMarkerChooserIndex), m_StatusBar(&m_State, &m_Season),
                           ...
                           })
{
    ...
    m_AddPlayerEvent.Subscribe([this](auto, std::any data) {
        ...
        m_Players.back().Init(&m_State, &m_Season, &m_HideCards, &m_RotateTurnEvent, &m_RestartBattleEvent,
...
void Game::Deserialize(StreamReader& r, Game& game)
{
    ...
    {
        player.Init(&game.m_State, &game.m_Season, &game.m_HideCards, &game.m_RotateTurnEvent,
                    &game.m_RestartBattleEvent, &game.m_TakeFavorMarkerEvent);
    }
...
void Game::Render() const
{
    ...
    if (m_State.Get() != State::SHOW_WINNER_PLAYER)
    {
        ...
        if (m_Players.size() > m_Turn)
            m_Players[m_Turn].Render(m_Assets);
        for (size_t i = 0; i < m_Players.size(); i++)
        {
            if (i != m_Turn)
                m_Players[i].Render(m_Assets);
        }
    ...
    else
    {
        ...
        {
            DrawText(TextFormat("%s wins", w.name.c_str()), Width, Height, 50, BLACK);
            Height += 30;
        }
...
void Game::FindRegionConquerer()
{
    ...
    {
        std::clog << "INFO: " << bm->GetRuler()->name << "conquered " << bm->GetName() << '\n';
    }
    potentialWinners.clear();
...
void Game::RestartBattle()
{
    ...
    m_State.Set(m_FavorMarkerChooserIndex < 0 ? State::PLACING_BATTLE_MARKER : State::PLACING_FAVOR_MARKER);
