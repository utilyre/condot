...
PauseMenu::PauseMenu(State* state, Event* quit, Event* save)
    : m_PauseMenuButton("Pause", Rectangle{GetScreenWidth() - 120.0f, 10, 120, 70}),
      m_ContinueButton("Continue",
                       Rectangle{(GetScreenWidth() - BUTTON_WIDTH) / 2.0f,
                                 (GetScreenHeight() - BUTTON_HEIGHT) / 2.0f - 120, BUTTON_WIDTH, BUTTON_HEIGHT}),
      m_HelpButton("Help", Rectangle{(GetScreenWidth() - BUTTON_WIDTH) / 2.0f,
                                     (GetScreenHeight() - BUTTON_HEIGHT) / 2.0f, BUTTON_WIDTH, BUTTON_HEIGHT}),
      m_ExitButton("Save & Exit",
                   Rectangle{(GetScreenWidth() - BUTTON_WIDTH) / 2.0f, (GetScreenHeight() - BUTTON_HEIGHT) / 2.0f + 120,
      ...
      m_State(state), m_QuitEvent(quit), m_SaveEvent(save), m_ShowHelpMenu(false)
{
}
...
void PauseMenu::Render(const AssetManager& assets) const
{
    if (m_State->Get() == State::MAIN_MENU || m_State->Get() == State::CUSTOMIZATION_MENU ||
        m_State->Get() == State::PLACING_BATTLE_MARKER || m_State->Get() == State::PLACING_FAVOR_MARKER)
    {
        return;
    }
    if (!m_ShowHelpMenu)
...
void PauseMenu::Update()
{
    ...
    else if (m_ContinueButton.Pressed() && m_State->Get() == State::PAUSE_MENU)
    {
        m_State->Set(m_State->GetPrev());
    }

    else if (m_HelpButton.Pressed() && m_State->Get() == State::PAUSE_MENU)
    {
        m_ShowHelpMenu = true;
    }

    else if (m_BackButton.Pressed() && m_State->Get() == State::PAUSE_MENU)
    {
        m_ShowHelpMenu = false;
    }

    else if (m_ExitButton.Pressed() && m_State->Get() == State::PAUSE_MENU)
    {
